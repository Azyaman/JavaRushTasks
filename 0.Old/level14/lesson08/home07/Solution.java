...
public class Solution
{
    ...
    public static void cleanAllApartaments(List<Apartament> apartaments)
    {
        ...
        for (Apartament item : apartaments)
        {
            if (item instanceof Apt1Room) {((Apt1Room) item).clean1Room();}
            if (item instanceof Apt2Room) {((Apt2Room) item).clean2Rooms();}
            if (item instanceof Apt3Room) {((Apt3Room) item).clean3Rooms();}
