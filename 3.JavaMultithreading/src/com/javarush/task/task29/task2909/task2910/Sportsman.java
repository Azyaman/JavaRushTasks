package com.javarush.task.task29.task2909.task2910;
...
public class Sportsman {
    ...
    public Sportsman(int number) {
        this.number = number;
        chanceToHit = BigDecimal.valueOf(ThreadLocalRandom.current().nextDouble(Constants.MIN_CHANCE_TO_HIT_IN_TARGET.doubleValue(), Constants.MAX_CHANCE_TO_HIT_IN_TARGET.doubleValue()));
    }

    public void calculateFullTime() {
        resultTime = resultTime.add(BigDecimal.valueOf(countHitOutTarget * Constants.PENALTY_TIME_PER_ONE_MISS));
        resultTime = resultTime.add(calculateTimeToRace());
    }

    private BigDecimal calculateTimeToRace() {
        return new BigDecimal(Constants.RACE_LENGTH / averageSpeed.doubleValue() * 3600).setScale(2, BigDecimal.ROUND_HALF_UP);
    }

    public void calculateNumberOfMiss() {
        for (int i = 0; i < Constants.NUMBER_OF_FIRE_LINES * 5; i++) {
