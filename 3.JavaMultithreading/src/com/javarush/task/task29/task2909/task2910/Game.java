...
public class Game {
    ...
    public Game() {
        ...
        for (int i = 0; i < Constants.SPORTSMAN_COUNT; i++) {
    ...
    private void initializeControlSportsman() {
        ...
        controlSportsman.setChanceToHit(Constants.MAX_CHANCE_TO_HIT_IN_TARGET);
    ...
    public void startCompetition() {
        for (int i = 0; i < members.size(); i++) {
            ...
            BigDecimal timeToRace = controlSportsman.getResultTime().subtract(BigDecimal.valueOf(members.get(i).getCountHitOutTarget() * Constants.PENALTY_TIME_PER_ONE_MISS));
            members.get(i).setAverageSpeed(new BigDecimal(Constants.RACE_LENGTH / (timeToRace.doubleValue() / 3600)).setScale(4, BigDecimal.ROUND_HALF_UP));
