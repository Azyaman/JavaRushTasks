...
public class Dog implements Pet {
    ...
    public Sayable toSayable(final int i) {
        class DogPet extends SuperDog implements Sayable {
            ...
            @Override
            public String say() {
                if (i < 1) { return String.format("%s спит.", getName());}
                else {
                    String str = "а";
                    String repeat = new String(new char[i]).replace("\0", str);
                    return String.format("%s лает г%sв! %s", getName(), repeat, formatter.format(new Date()));
                }
            }
